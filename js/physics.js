...
(function (root, factory) {
  ...
  function snapEndpoint(raw, level) {
    ...
    for (const { a, b } of surfaces) {
      const q = { x: Math.max(a.x, Math.min(b.x, p.x)), y: a.y };
  ...
  function validateStroke(level, raw) {
    ...
    if (sampled.some(p => p.x < 24 || p.x > 696 || p.y < level.chair[0].y - 4 || p.y > 465)) {
      return { valid: false, code: 'bounds', message: '只能在椅面及以下的画区补线，不要画到人身上。', ink };
    }
    ...
    return { valid: true, ink, points, attached: level.chair.some(r => hits(points, r, RADIUS + 1)) };
  }
  ...
  function createTrial(level, raw) {
    ...
    const engine = Engine.create({ positionIterations: 10, velocityIterations: 8, enableSleeping: false });
    engine.gravity.y = 1;
    ...
    return {
      valid: true, engine, level, stroke, chair, looseInk, origin, inkOrigin, terrain, catZones,
  ...
  function contacts(trial) {
    ...
    for (const pair of trial.engine.pairs.list) {
      ...
      if (!((a === trial.chair && b.isStatic) || (b === trial.chair && a.isStatic))) continue;
  ...
  function step(trial) {
    ...
    if (trial.catZones.some(zone => Query.collides(zone, [trial.chair, ...(trial.looseInk ? [trial.looseInk] : [])]).length)) {
      fail(trial, 'cat', '结构碰到了粉色猫区。线可以绕弯，但要给晃动也留一点余地。');
    }
    if (tilt > 24) {
      fail(trial, trial.stroke.attached ? 'tipped' : 'detached', trial.stroke.attached
        ? '重心跑出了落脚范围，椅子翻了。试着把脚落得更稳、更靠外；空洞不能承重。'
        : '这一笔没接上木椅，松手后只是自由落体。让笔画碰到椅面或旧椅脚。');
    }
    if (seat.y > trial.level.chair[0].y + 65 || seat.x < 40 || seat.x > 680) {
      fail(trial, 'fell', '支撑没有接住重量，座面掉下去了。脚尖要碰到实心表面，不是停在空中。');
    }
    ...
    if (trial.elapsed >= DURATION - 0.01) {
      ...
      if (!trial.outcome) {
        if (tilt > 10 || trial.stableMs < 700 || !trial.diagnostics.contacts.length) {
          fail(trial, 'wobbly', '五秒到了，还没坐稳。稍微扩大落脚范围，并让脚尖贴住承重表面。');
        } else {
  ...
  function simulate(level, points) {
    ...
    return { ...trial.outcome, valid: true, attached: trial.stroke.attached, contacts: trial.diagnostics.contacts, position: { ...trial.chair.position } };
  }
  return { DT, DURATION, RADIUS, zones, activeForces, length, distance, segmentRect, simplify, snapEndpoint, validateStroke, createTrial, transform, step, simulate };
});
